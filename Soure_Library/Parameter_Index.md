...
|$A_DBB[7]|DB4900.DBX7.0|自动对刀接近开关回退到位(蜗杆接近开关为首测)
|$A_DBB[8]|DB4900.DBX8.0|自动对刀接近开关伸出到位(蜗杆接近开关为首测)
...
|$A_DBB[15]|DB4900.DBX15.0|自动对刀测头回退到位(蜗杆测头为二次侧量)
|$A_DBB[16]|DB4900.DBX16.0|自动对刀测头伸出到位(蜗杆测头为二次测量)
...
|INI[41]||X轴工件夹装安全位置
|INI[42]||(外螺纹自动对刀)砂轮中心与测头1中心重合时Z轴坐标
|INI[43]||耳朵深度
...
|INI[76]||蜗杆模数输入
|INI[77]||上一次新砂轮直径缓存
|INI[78]||(外螺纹自动对刀)砂轮中心与测头2中心重合时Z轴坐标
|||
|**对刀(TOOL_SET)**||
